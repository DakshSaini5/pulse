...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-[#0B0F19] text-slate-100 flex flex-col relative overflow-x-hidden selection:bg-primary selection:text-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-8">
            <Link to="/" className="flex items-center gap-2 group">
              ...
              <span className="font-bold text-xl tracking-tight bg-gradient-to-r from-white via-slate-200 to-slate-400 bg-clip-text text-transparent">Pulse</span>
            </Link>
          ...
          <div className="flex items-center gap-4">
            {user ? (
              <>
                ...
                <div className="relative">
                  <button 
                    ...
                    className="p-2.5 rounded-xl border border-white/5 hover:border-white/10 bg-white/5 hover:bg-white/10 transition-all text-slate-400 hover:text-white"
                  >
                  ...
                  {/* Dropdown menu */}
                  {notificationsOpen && (
                    <div className="absolute right-0 mt-3 w-80 glass-panel border border-white/10 rounded-2xl p-4 shadow-xl z-50 text-slate-200">
                      ...
                      <div className="space-y-3">
                        <div className="p-2 bg-success/10 border border-success/20 rounded-lg text-xs">
                          <p className="font-medium text-white mb-0.5">Analysis Ready</p>
                          <p className="text-slate-400">Medical Report CBC analysis parsed perfectly. Route specialist updated.</p>
                        </div>
                        <div className="p-2 bg-primary/10 border border-primary/20 rounded-lg text-xs">
                          <p className="font-medium text-white mb-0.5">Welcome to Pulse</p>
                          <p className="text-slate-400">Start discovering hospitals near your location.</p>
                        </div>
                      </div>
                    </div>
                  )}
                ...
                <div className="hidden sm:flex items-center gap-3 pl-4 border-l border-white/10">
                  <div className="w-9 h-9 rounded-xl bg-slate-800 flex items-center justify-center border border-white/10 text-slate-200 font-bold text-sm">
                ...
                <button
                  ...
                  className="p-2.5 rounded-xl border border-white/5 hover:border-danger/20 bg-white/5 hover:bg-danger/10 hover:text-danger transition-all text-slate-400"
                  title="Logout"
                >
              ...
              <div className="flex items-center gap-2">
                <Link
                  to="/login"
                  className="px-4 py-2 rounded-xl text-sm font-medium border border-white/10 hover:border-white/20 bg-white/5 hover:bg-white/10 transition-all flex items-center gap-1.5"
                >
            ...
            <button
              ...
              className="lg:hidden p-2 rounded-lg border border-white/10 bg-white/5 text-slate-300"
            >
      ...
      {/* Mobile Drawer */}
      {mobileMenuOpen && (
        <div className="lg:hidden sticky top-[64px] z-30 w-full glass-panel border-b border-white/10 p-4 animate-fade-in text-slate-300">
      ...
      {/* Footer */}
      <footer className="w-full glass-panel border-t border-white/5 mt-auto py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-2">
            <span className="font-bold text-lg bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent">Pulse</span>
            <span className="text-xs text-slate-500">© 2026. All rights reserved.</span>
          </div>
          <div className="flex gap-6 text-xs text-slate-400">
            <Link to="/about" className="hover:text-white transition-colors">About Us</Link>
            <Link to="/contact" className="hover:text-white transition-colors">Contact</Link>
            <Link to="/pricing" className="hover:text-white transition-colors">SaaS Pricing</Link>
