  ...
  return (
    <div className="space-y-8 pb-20 relative">
      <div className="text-left space-y-2">
        ...
        <p className="text-xs text-slate-500">Discover hospitals matching your specialty need, distance parameters, and emergency situations.</p>
      </div>

      {/* Filters Segment */}
      <form onSubmit={handleSearchSubmit} className="glass-panel rounded-2xl p-4 sm:p-6 border border-slate-200 grid grid-cols-1 md:grid-cols-12 gap-4 items-end text-left bg-white/[0.01]">
        <div className="md:col-span-4 space-y-1">
          <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Hospital Name / Keywords</label>
        ...
        <div className="md:col-span-3 space-y-1">
          <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Clinical Specialty</label>
          <select
            value={specialty}
        ...
        <div className="md:col-span-2 space-y-1">
          <div className="flex justify-between items-center">
            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Max Radius</label>
        ...
        <div className="md:col-span-2 grid grid-cols-2 gap-3">
          ...
          <button
            type="submit"
            className="py-3 px-3 rounded-xl bg-primary hover:bg-primary-hover text-slate-900 text-[11px] font-bold shadow-md shadow-primary/25 transition-all flex items-center justify-center gap-1.5"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5 space-y-4 max-h-[600px] overflow-y-auto pr-2 text-left">
          {loading ? (
            <div className="space-y-4">
              {[1, 2, 3].map((n) => (
                <div key={n} className="glass-panel rounded-2xl p-5 border border-slate-200 h-32 animate-pulse flex flex-col justify-between">
            ...
            <div className="glass-panel rounded-2xl p-10 border border-slate-200 text-center text-slate-500">
                ...
                >
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-4 text-xs font-semibold">
                      ...
                      <span className="text-[10px] bg-slate-800 border border-slate-200 text-slate-600 px-2 py-0.5 rounded-full font-medium">
        ...
        <div className="lg:col-span-7 h-[600px] rounded-2xl overflow-hidden shadow-2xl relative border border-slate-200">
          <Map
      ...
      {compareIds.length > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[2000] w-full max-w-lg px-4 animate-slide-up">
          <div className="glass-panel border border-primary/30 bg-slate-50/90 backdrop-blur-xl p-4 rounded-2xl flex items-center justify-between shadow-2xl">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-xl bg-primary/15 flex items-center justify-center text-primary">
                <Layers className="w-5 h-5 animate-pulse" />
              </div>
              <div className="text-left">
                <span className="text-xs font-bold text-slate-900 block">Compare Panel Active</span>
                <span className="text-[10px] text-slate-500">{compareIds.length} hospital{compareIds.length > 1 ? 's' : ''} selected (Max 3)</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="px-4 py-2 bg-primary hover:bg-primary-hover text-slate-900 text-[10px] font-bold rounded-xl flex items-center gap-1 shadow-md shadow-primary/25 transition-all"
              >
