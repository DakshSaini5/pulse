  ...
  return (
    <div className="space-y-8 pb-16 text-left">
      <div className="text-left space-y-2">
        ...
        <p className="text-xs text-slate-500">Manage and browse clinics or medical departments you bookmarked for later navigation.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="glass-panel rounded-3xl p-12 text-center max-w-md mx-auto my-12 border border-slate-200">
          ...
          <h2 className="text-xl font-bold text-slate-900">No Saved Hospitals</h2>
          <p className="text-xs text-slate-500 mt-2">Bookmarked clinics from the search map will appear here for fast routing access.</p>
          <Link to="/search" className="mt-6 px-4 py-2 bg-primary text-slate-900 rounded-xl text-xs font-semibold inline-block">Explore Maps</Link>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {hospitals.map(hosp => (
            <div
              ...
              className="glass-panel glass-panel-hover rounded-3xl p-6 border border-slate-200 flex flex-col justify-between cursor-pointer text-left relative bg-white/[0.005]"
            >
              <div className="space-y-3">
                <div className="flex justify-between items-start gap-4">
                  <div>
                    <h3 className="font-extrabold text-slate-900 text-lg leading-snug">{hosp.name}</h3>
              ...
              <div className="flex justify-between items-center pt-4 border-t border-slate-200 mt-4 text-[10px] text-slate-500 font-bold uppercase tracking-wider">
