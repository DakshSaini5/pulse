  ...
  return (
    <div className="space-y-8 pb-16 text-left">
      <div className="text-left space-y-2">
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500">Manage and browse clinics or medical departments you bookmarked for later navigation.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="glass-panel rounded-3xl p-12 text-center max-w-md mx-auto my-12 border border-slate-200 dark:border-slate-700">
          <Heart className="w-12 h-12 text-slate-600 dark:text-slate-300 mx-auto mb-4" />
          <h2 className="text-xl font-bold text-slate-900 dark:text-white">No Saved Hospitals</h2>
          <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500 mt-2">Bookmarked clinics from the search map will appear here for fast routing access.</p>
          <Link to="/search" className="mt-6 px-4 py-2 bg-primary text-slate-900 dark:text-white rounded-xl text-xs font-semibold inline-block">Explore Maps</Link>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {hospitals.map(hosp => (
            ...
            >
              <div className="space-y-3">
                <div className="flex justify-between items-start gap-4">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500 flex items-center gap-1 mt-1 leading-none">
                      <MapPin className="w-3.5 h-3.5 text-slate-500 dark:text-slate-400 dark:text-slate-500" />
                      {hosp.address}
                    </p>
                  </div>
              ...
              <div className="flex justify-between items-center pt-4 border-t border-slate-200 dark:border-slate-700 mt-4 text-[10px] text-slate-500 dark:text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider">
